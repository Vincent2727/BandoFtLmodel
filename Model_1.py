...
class BandoFtl:
    ...
    def plot_figure(self, position_history, velocity_history, time, vel_std):
        ...
        print(f"Saving Figures with the extension: ..{self.differnce_vel_var}_{distraction}.png")
        # Plot positions
        ...
        # plt.autoscale()
        plt.tight_layout()
        name = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/pos{self.differnce_vel_var}_{distraction}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/vel{self.differnce_vel_var}_{distraction}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/pos_rel{self.differnce_vel_var}_{distraction}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/vel_rel{self.differnce_vel_var}_{distraction}.png'
        plt.savefig(name)
        ...
        if len(self.wave_started) >= self.num_vehicles:
            ...
            #plt.legend()
            plt.tight_layout()
            name = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/Wave_Started_rel_vel{self.differnce_vel_var}_{distraction}.png'
            plt.savefig(name)
...
manager.pertubance = True # Distraction effect
...
go = True
if go:
    for i in values: # Revert back to; for i in values:
        # print(i)
        manager.differnce_vel_var = i # Revert back to i, 
        manager.initialize()
        manager.run_system(plot_yn=False)
        # For printing
 
        print(f"The wave started at: {manager.wave_started[step]}s; Threshold: {manager.wave_threshold}; Velocity Perturbation: {i}") # Revert back to {i}
...
print(f'The initial velocity perturbations of the leading velocity were the range ({values[0]}, {values[-1]}), with a step count of 100.')
...
if plot:
    if manager.pertubance:
        distractionf = 'Distraction3'
    else:
        distractionf = 'NoDistraction3'
    ...
    plt.xlabel('Velocity Perturbation (m)')
    plt.ylabel('Time Wave Started (s)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    namef = f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/WaveStartedvsPerturbation{values[-1]}_{distractionf}.png'
    plt.savefig(namef)
