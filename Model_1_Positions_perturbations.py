...
class BandoFtl:
    ...
    def __init__(self, parameters, time_horizon):
        ...
        self.folder = f"/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/{self.timeset}_PosPerturbation"
    ...
    def plot_figure(self, position_history, velocity_history, time, vel_std):
        if self.pertubance:
            ...
            distraction = 'NoDistraction'

        folder_path = f"/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/{self.timeset}_PosPerturbation"

        self.folder = folder_path

        os.makedirs(folder_path, exist_ok=True)

        print(f"Saving Figures with the extension: .._{self.initial_offset:.02f}_{distraction}_{self.timeset}.png")
        # Plot positions
        ...
        # plt.autoscale()
        plt.tight_layout()
        name = f'{folder_path}/pos_{self.initial_offset:.02f}_{distraction}_{self.timeset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'{folder_path}/vel_{self.initial_offset:.02f}_{distraction}_{self.timeset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'{folder_path}/pos_rel_{self.initial_offset:.02f}_{distraction}_{self.timeset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'{folder_path}/vel_rel_{self.initial_offset:.02f}_{distraction}_{self.timeset}.png'
        plt.savefig(name)
        ...
        csv_filename = f'{folder_path}/statistics_{self.initial_offset:0.2f}_{distraction}_{self.timeset}.csv'
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'{folder_path}/statistics_{self.initial_offset:0.2f}_{distraction}_{self.timeset}.png')
        ...
        #     #plt.legend()
        #     plt.tight_layout()
        #     name = f'{folder_path}/Wave_Started_rel_vel_posOffset{self.initial_offset}_{distraction}_{self.timeset}.png'
        #     plt.savefig(name)
...
manager.pertubance = True # Distraction effect: True is on, False is off
...
manager.timeset = timeset


if go:
    for i in values:
        # print(i)
        manager.initial_offset = i
        manager.initialize()
        manager.run_system(plot_yn=False)
        # For printing
 
        print(f"The wave started at: {manager.wave_started[step]}s; Threshold: {manager.wave_threshold}; Position Perturbation: {i}")
...
print(f"The initial position perturbations of the leading vehicle's position were the range ({values[0]}, {values[-1]}), with a step count of {step_count}.")
on = ''
...
if plot:
    ...
    # try:
    #     with open(f'/Users/vincentdegaetano/Documents/VsCodeProjects/Traffic_Images/Model_1_Images/WaveStartedvsPerturbation{values[-1]}_{distractionf}.png') as file:
    #         pass
    #     distractionf = distractionf + "2"
    ...
    plt.title('Wave Started vs Initial Position Perturbation')
    plt.xlabel('Position Perturbation (m)')
    plt.ylabel('Time Wave Started (s)')
    plt.grid(True)
    # plt.legend()
    plt.tight_layout()
    namef = f'{manager.folder}/WaveStartedvsPerturbationOffset_{values[-1]}_{distractionf}_{timeset}.png'
    plt.savefig(namef)
