...
class BandoFtl:
    ...
    def __init__(self, parameters, time_horizon):
        ...
        self.differnce_vel_var = 0
        # Variable for the tracking of the waves
        self.current_best = 1

        # Variables for the intial Position change
        self.initial_offset = 0 # MUST BE LESS THAN THE CAR LENGTH TO AVOID CRASHING BEFORE STARTING
    ...
    def initialize(self):
        ...
        print(f"Changed the ego vehicle's positions by {self.initial_offset}")
    ...
    def rk4(self, init_positions, init_velocities, params, dt):
        ...
        for i in range(num_steps):
            ...
            # Varaince for the velocity at the current time
            variance = np.nanvar(velocities_curr)

            # Check the variance at each point against the threshold
    ...
    def plot_figure(self, position_history, velocity_history, time, vel_std):
        ...
        # plt.autoscale()
        plt.tight_layout()
        name = f'pos{self.initial_offset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'vel{self.initial_offset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'pos_rel{self.initial_offset}.png'
        plt.savefig(name)
        ...
        plt.legend()
        plt.tight_layout()
        name = f'vel_rel{self.initial_offset}.png'
        plt.savefig(name)
        ...
        if len(self.wave_started) >= self.num_vehicles:
            ...
            #plt.legend()
            plt.tight_layout()
            name = f'Wave_Started_rel_vel{self.initial_offset}.png'
            plt.savefig(name)
...
values = [-0.05,- 0.1,- 0.5, -1.0, -1.5,-2.5,- 3, -3.5, 0.05, 0.1, 0.5, 1.0, 1.5,2.5, 3, 3.5]
...
if go:
    for i in values:
        ...
        print(f"The wave started at: {started}s; Threshold: {manager.wave_threshold}; Position Perturbation: {i}")
